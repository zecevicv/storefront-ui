...
<template>
  <form class="grid grid-cols-1 md:grid-cols-[50%_1fr_120px] gap-4" data-testid="address-form"
    ...
    <label>
      ...
      <SfInput v-model="addressFormFieldsInput.name" name="name" autocomplete="given-name" required
        :placeholder="$t('form.NameLabel')" />
    </label>
    <label class="md:col-span-2">
      <UiFormLabel>{{ $t("form.streetNameLabel") }}</UiFormLabel>
      <SfInput v-model="addressFormFieldsInput.street" name="streetName" autocomplete="family-name" required
        :placeholder="$t('form.streetNameLabel')" />
    </label>
    <label class="md:col-span-3">
      <UiFormLabel>{{ $t("form.phoneLabel") }}</UiFormLabel>
      <SfInput v-model="addressFormFieldsInput.phone" name="phone" type="tel" autocomplete="tel" required
        :placeholder="$t('form.phoneLabel')" />
    </label>

    <UiFormSelectCountries v-model="addressFormFieldsInput.countryId" />
    <UiFormSelectStates v-if="addressFormFieldsInput.countryId != 0" :key="addressFormFieldsInput?.countryId"
      v-model="addressFormFieldsInput.stateId" :country-id="addressFormFieldsInput?.countryId" />
    <label class="md:col-span-2">
      <UiFormLabel>{{ $t("form.cityLabel") }}</UiFormLabel>
      <SfInput v-model="addressFormFieldsInput.city" name="city" autocomplete="address-level2" required
        :placeholder="$t('form.cityLabel')" />
    </label>
    <label>
      <UiFormLabel>{{ $t("form.postalCodeLabel") }}</UiFormLabel>
      <SfInput v-model="addressFormFieldsInput.zip" name="postalCode" autocomplete="postal-code" required
        :placeholder="$t('form.postalCodeLabel')" />
    </label>
    <label v-if="!isEditForm && type === AddressEnum.Billing" class="md:col-span-2">
      <SfCheckbox v-model="billingAddresIsTheSameAsShipping" name="billingAddresIsTheSameAsShipping" />
