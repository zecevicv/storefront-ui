          ...
          <li role="none">
            <transition enter-active-class="transform transition duration-500 ease-in-out"
              ...
              <SfDrawer ref="drawerRef" v-model="isOpen" disable-click-away placement="top"
                ...
                <div>
                  <div v-for="{ name, childs, slug } in categoriesForMegaMenu" :key="name" class="py-2 text-black">
                    <NuxtLink :to="slug" role="presentation"
                    ...
                    <ul>
                      <li v-for="{ name, slug, childs: subcategory } in childs" :key="name">
                        <SfListItem v-if="subcategory !== null" tag="a" :href="slug" size="sm" role="none"
