  ...
  <div>
    ...
    <header ref="menuRef"
      ...
      <div class="flex items-center jfustify-between lg:justify-start h-full w-full narrow-container">
              ...
              <transition enter-active-class="transform transition duration-300 ease-in-out"
                ...
                <SfDrawer ref="drawerRef" v-model="isOpen" disable-click-away placement="top"
                  ...
                  <div class="grid grid-cols-1 lg:gap-x-6 lg:grid-cols-4 lg:narrow-container lg:relative">
                    <div v-for="{ name, childs, id, slug } in categoriesForMegaMenu" :key="id"
                      ...
                      <ul>
                        <li v-for="child in childs" :key="child.id">
                          <SfListItem v-if="child.childs !== null" tag="span" :to="child.slug" size="sm" role="none"
                            class="typography-text-base lg:typography-text-sm py-4 lg:py-1.5" @click="goTo(child.slug)">
                            {{ child.name }}
                          </SfListItem>
                        </li>
                      </ul>
                  ...
                  <SfButton square size="sm" variant="tertiary" aria-label="Close navigation menu"
                    class="absolute right-5 top-5 hover:bg-white active:bg-white" @click="close()">
        ...
        <form ref="formSearchTemplateRef" role="search"
          class="hidden lg:flex flex-[100%] mt-2 md:mt-0 md:ml-10 pb-2 md:pb-0 relative w-full" @submit.prevent>
          <SfInput v-model="searchInputValue" type="text" class="[&::-webkit-search-cancel-button]:appearance-none"
            placeholder="Search" wrapper-class="flex-1 h-10 pr-0" size="base" @input="search()"
